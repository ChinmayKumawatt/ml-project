    ...
    def initiate_model_trainer(self,train_arr,test_arr):
        try:
            ...
            models_report:dict = evaluate_models(X_train=X_train,y_train = y_train,X_test = X_test,y_test =y_test,
                                                 models = models)
            best_models_score = max(sorted(models_report.values()))

            best_model_name = list(models_report.keys())[
                list(models_report.values()).index(best_models_score)
            ]
            ...
            logging.info("Found the best model and trained the models")
            save_object(
